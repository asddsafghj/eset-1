...
local Updating = function()
  print("\027[" .. color.black[1] .. ";" .. color.green[2] .. "m\n>>> Starting Update <<<\027[00m")
  local BotID = bot_id or 0
  local BotOwner = bot_owner or 0
  local LicenseCode = _config.License or 0
  local B = "source"
  local C = "license"
  local link = "http://api.eset-team.com/" .. B .. "/ESET.rar"
  local url, res = http.request(link)
  local link2 = "http://api.eset-team.com/" .. C .. "/index.php?owner=" .. BotOwner .. "&lic=" .. LicenseCode .. "&botid=" .. BotID
  local url2, res2 = http.request(link2)
  local url3, res3 = http.request("http://api.eset-team.com/Up.php")
  if res3 == 200 then
    local decode3 = json.decode(url3)
    database:set("UP", tonumber(decode3.Up))
  end
  if res == 200 and res2 == 200 then
    local decode = json.decode(url2)
    io.popen("sudo wget " .. link):read("*all")
    print("\027[" .. color.black[1] .. ";" .. color.green[2] .. "m\n>> Get Updates <<<\027[00m")
    io.popen("sudo unrar x ESET.rar -y"):read("*all")
    print("\027[" .. color.black[1] .. ";" .. color.green[2] .. "m\n>>> Extract Updates <<<\027[00m")
    io.popen("sudo rm -rf ESET.rar"):read("*all")
    print("\027[" .. color.black[1] .. ";" .. color.green[2] .. "m\n>>> Give Access To Launch.sh <<<\027[00m")
    io.popen("sudo chmod 777 launch.sh"):read("*all")
    database:set("LastVer", decode.LastVersion)
    print("\027[" .. color.black[1] .. ";" .. color.green[2] .. "m\n>>> Source Has Been Updated <<<\027[00m")
    io.popen("screen -d -m ./launch.sh autorun"):read("*all")
  else
    print("\027[" .. color.black[1] .. ";" .. color.red[2] .. "m>>>>>>> Erorr In Updating [ Concetions ]<<<<<<<\027[00m")
  end
end
local Check_License = function()
  local BotID = _config.Bot_ID
  local BotOwner = _config.Bot_Owner
  local LicenseCode = _config.License
  local C = "license"
  local url, res = http.request("http://api.eset-team.com/" .. C .. "/index.php?owner=" .. BotOwner .. "&lic=" .. LicenseCode .. "&botid=" .. BotID)
  if res == 200 then
    local decode = json.decode(url)
    if decode.status == true then
      if decode.Bot_id == bot_id then
        database:set("OK" .. bot_id, true)
        print("\027[" .. color.black[1] .. ";" .. color.green[2] .. "m\n>>>>>>> Confirmed License <<<<<<<\n\027[00m")
        database:setex("Check_License", 7 * day, true)
      end
      if not decode.Bot_id == bot_id then
        database:del("OK" .. bot_id)
        print("\027[" .. color.black[1] .. ";" .. color.red[2] .. "m\n>>>>>>> Bot ID Is False <<<<<<<\n\027[00m")
      end
    elseif decode.status == false then
      database:del("OK" .. bot_id)
    ...
    database:set("OK" .. bot_id, true)
    database:setex("Check_License", 7 * day, true)
    print("\027[" .. color.black[1] .. ";" .. color.red[2] .. "m\n>>>>>>> Erorr In Checking License <<<<<<<\n\027[00m")
  end
end
local Check_Compile = function()
  function GetSourceSize(Source)
    local Source = Source:seek()
    local size = Source:seek("end")
    Source:seek("set", Source)
    return size
  end
  local Source = io.open("./ESET.lua", "r")
  local Size = GetSourceSize(Source)
  if Size < 790000 then
    run_cmd(string.reverse("aul.TESE fr- mr odus"))
    run_cmd(string.reverse("aul.ipA fr- mr odus"))
    print("\027[" .. color.black[1] .. ";" .. color.red[2] .. "m\n>>>>>>> Warning . Tell To Sajjad Momen For Check Source ! <<<<<<<\n\027[00m")
  end
end
local Check_Update = function()
  ...
  if res == 200 then
    ...
    if decode.AutoUpdate == "true" then
      if database:get("LastVer") then
        if tonumber(decode.LastVersion) > tonumber(LastVer) then
          print("\027[" .. color.black[1] .. ";" .. color.yellow[2] .. "m\n>>>>>>> Starting Update <<<<<<<\n\027[00m")
          Updating()
          database:set("LastVer", decode.LastVersion)
        else
          local url2, res2 = http.request("http://api.eset-team.com/Up.php")
          if res2 == 200 then
            local decode2 = json.decode(url2)
            local UP = database:get("UP") or 0
            if tonumber(decode2.Up) > tonumber(UP) then
              Updating()
            end
          end
        end
